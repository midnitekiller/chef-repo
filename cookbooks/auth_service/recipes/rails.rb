...
s = node[:auth_service]                             # Node (s)ervice data
...
application s[:app_name] do
  ...
  symlink_before_migrate({"log"                => "log",
                          ...
                          "seeding_data.yml"   => "config/seeding_data.yml" #,
                       #  "aws.yml"            => "config/aws.yml"
                         })

  # It's unclear if we need a symlinks() with the same args, for instances where migration
  # is *not* run.
...
# Cleanup cron task
cron "authentication_cleanup" do
  minute "0"
  command "/usr/bin/curl -X PUT http://127.0.0.1/cleanup"
end
