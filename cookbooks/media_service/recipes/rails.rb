...
s = node[:media_service]                            # Node (s)ervice data
...
template "#{s[:app_dir]}/shared/x_riak_constants.rb" do
  source "x_riak_constants.rb.erb"
  owner node[:ocean][:rails_deploy_user]
  group node[:ocean][:rails_deploy_group]
  mode "0664"

  riak_role = b['riak_server_role']
  riak_node_ips = search(:node, "chef_environment:#{node.chef_environment} AND role:#{riak_role}").
    collect { |node| node.ipaddress }

  # Set up the Riak IPs. If deploying to TeamCity, use the master environment servers for tests.
  variables(riak_node_ips: riak_node_ips, 
            chef_env: node.chef_environment)
  #          chef_env: (node.chef_environment == 'ci' ? 'master' : node.chef_environment)) 
end
...
application s[:app_name] do
  ...
  purge_before_symlink [".git", ".gitignore", ".idea", ".rspec", 
                        ...
                        "config/initializers/x_riak_constants.rb"
                        ]
  ...
  symlink_before_migrate({"log"                 => "log",
                          ...
                          "x_riak_constants.rb" => "config/initializers/x_riak_constants.rb",
